...
class TourPlanAdmin(admin.ModelAdmin):
    list_display = ['title', 'max_adults', 'max_children', 'max_infants', 
                    'price_adult', 'is_active', 'created_at']
...
class TimeSlotInline(admin.TabularInline):
    model = TimeSlot
    extra = 1
    fields = ['start_time', 'end_time', 'available_adults', 
              'available_children', 'available_infants', 'is_active']
...
class TimeSlotAdmin(admin.ModelAdmin):
    list_display = ['tour_date', 'start_time', 'end_time', 
                    'available_adults', 'available_children', 
                    'available_infants', 'is_active']
...
class CartItemAdmin(admin.ModelAdmin):
    list_display = ['user', 'tour_plan', 'time_slot', 
...
class BookingItemInline(admin.TabularInline):
    model = BookingItem
    extra = 0
    fields = ['tour_plan', 'time_slot', 'num_adults', 'num_children', 
              'num_infants', 'item_price']
    readonly_fields = ['tour_plan', 'time_slot', 'num_adults', 
                       'num_children', 'num_infants', 'item_price']


@admin.register(Booking)
class BookingAdmin(admin.ModelAdmin):
    list_display = ['id', 'user', 'total_price', 'status', 
                    'booked_by_admin', 'created_at']
...
class BookingItemAdmin(admin.ModelAdmin):
    list_display = ['booking', 'tour_plan', 'time_slot', 
                    'num_adults', 'num_children', 'num_infants', 
                    'item_price', 'created_at']
    list_filter = ['created_at', 'tour_plan']
